...
class WeatherViewController: UIViewController, CLLocationManagerDelegate, ChangeCityDelegate {
    ...
    @IBAction func toggleOff(_ sender: UISwitch) {
        let temp = Double(weatherDataModel.temperature)
        if sender.isOn{
            weatherDataModel.temperature = Int((temp - 32) * (5/9))
        }else{
            weatherDataModel.temperature = Int((temp * (9/5)) + 32)
        }
